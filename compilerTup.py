...
class CompilerTup(CompilerWhile):
    ...
    def isTuple(self, arg: TupleType) -> bool:
        match arg:
            case TupleType(t):
                return True
            case _:
    ...
    def select_stmt(self, s: stmt) -> List[instr]:
        match s:
            ...
            case Assign([arg1], Allocate(arg2, TupleType(types))):
                ...
                count = 7
                for t in types:
                    type_tag |= bool2int(self.isTuple(t)) << count
                    count += 1
        ...
        match i:
            case Callq(name, args):
                if name == label_name('collect'):
                    for v in afterSet:
                        if v not in self.registers and self.isTuple(self.var_types[v.id]):
    ...
    def allocate_registers(self, colors: Dict[Variable : int]):
        ...
        for k,v in colors.items():
            ...
            elif not self.isTuple(self.var_types[k.id]):
                loc = -8 - 8*(v - 11)
